...
export default function AddItemPage() {
    ...
    try {
      const newImages = await Promise.all(
        Array.from(files).map(async (file) => {
          ...
          const { error: uploadError } = await supabase.storage.from("item-images").upload(filePath, file, { cacheControl: "3600", upsert: true });
  ...
  const removeImage = async (index: number) => {
    ...
    if (imageToRemove.url.startsWith("blob:")) {
      ...
      const { error } = await supabase.storage.from("item-images").remove([imageToRemove.filepath]);
  ...
  const fetchItemDetails = async (itemId: string) => {
    try {
      ...
      const { data: categoriesData, error: categoriesError } = await supabase.from("categories").select("*").order("display_order", { ascending: true });
      if (categoriesError) throw categoriesError;

      const { data: item, error: itemError } = await supabase.from("items").select("*, categories:category_id (id, name, parent_id)").eq("id", itemId).single();
      if (itemError) throw itemError;

      const { data: imageData, error: imageError } = await supabase.from("item_images").select("*").eq("item_id", itemId).order("display_order");
      ...
      const processedImages = await Promise.all(
        imageData.map(async (img) => {
          ...
          return { url: img.image_url, file, filepath: `duplicate/${Date.now()}-${file.name}` };
        })
      );
        ...
        <CardContent>
          <div className="w-full max-w-2xl mx-auto">
            ...
            {currentView === "camera" && (
              <CameraView
                images={images}
                onAddImage={(newImage) => setImages((prev) => [...prev, newImage])}
