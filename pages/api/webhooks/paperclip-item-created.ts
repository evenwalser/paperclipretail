...
//         (resolve, reject) =>
//           form.parse(req as any, (err, f, fi) =>
...
export default async function handler(
  ...
  const signature = req.headers["x-paperclip-signature"] as string;
  if (!signature) {
    return res.status(400).json({ error: "Missing signature" });
  }
  const secret = process.env.PAPERCLIP_WEBHOOK_SECRET;
  if (!secret) {
    console.error("Missing PAPERCLIP_WEBHOOK_SECRET");
  ...
  if (contentType.includes("application/json")) {
    ...
    try {
      ({ fields, files } = await new Promise<{ fields: Fields; files: Files }>(
        (resolve, reject) =>
          form.parse(fakeReq as any, (err, f, fi) =>
            err ? reject(err) : resolve({ fields: f, files: fi })
          )
      ));
    } catch (err) {
